...
const safeMapPinActionPattern = /^(?:set|choose) (?:(?:a|the|this|that) )?pin (?:(?:on|onto|for) (?:(?:the|this|that) )?map|(?:at|near) [\p{L}\p{N}]+(?: [\p{L}\p{N}]+)* (?:on|onto) (?:(?:the|this|that) )?map)$/iu;
